...
class ProfileForm(forms.Form):    
    # def validate_unique(self):
        # pprint.pprint(self)
        # userScreenId = self
        # SearchResult =  CustomUser.objects.filter(user_screen_id=userScreenId).all().exclude(id=)
        # if SearchResult.exists():
        #     raise forms.ValidationError(f"{userScreenId}は既に使われています。別のIDに変更してください")
        #     # raise self.add_error('screenId', f"{userScreenId}は既に使われています。別のIDに変更してください")
        # return userScreenId
